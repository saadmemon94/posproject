        ...
        <div id="receipt-data">
            ...
            <table class="table-bordered">
                ...
                <tbody>
                    ...
                    $total_packets_quantity = 0;
                    $total_cartons_quantity = 0;
                    ?>
                    @foreach ($products as $product)
                        <?php
                        $product_name = $product->product_name;
                        $product_cartons_available = $product->product_cartons_available;
                        $product_packets_available = $product->product_packets_available;
                        $product_pieces_available = $product->product_pieces_available;
                        ...
                        <tr class="row">
                            ...
                            <td colspan="3" class="mycol form-col-2" style="text-align:left;">
                                @if ($product_cartons_available !== 0)
                                    {{ number_format((int) $product_cartons_available) . 'crtns ' }}
                                @endif
                                @if ($product_packets_available !== 0)
                                    {{ number_format((int) $product_packets_available) . 'pckts ' }}
                                @endif
                                @if ($product_pieces_available !== 0)
                                    {{ number_format((int) $product_pieces_available) . 'pcs' }}
                                @endif
                            </td>
                            <?php $total_pieces_quantity += $product_pieces_available; ?>
                            <?php $total_packets_quantity += $product_packets_available; ?>
                            <?php $total_cartons_quantity += $product_cartons_available; ?>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr class="row">
                        ...
                        <th colspan="3" class="mycol form-col-2" style="text-align:left;">
                            {{ number_format((int) $total_cartons_quantity) }}{{ __('crtns') }}
                            {{ number_format((int) $total_packets_quantity) }}{{ __('pckts') }}
                            {{ number_format((int) $total_pieces_quantity) }}{{ __('pcs') }}
                        </th>
                    </tr>
                </tfoot>
            </table>
